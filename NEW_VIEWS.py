...
from apps.food_diary.schemas import (
    ...
    CreateMealSuccessResponse, UpdateMealSuccessResponse,
    GetMealSuccessResponse, GetMealsSuccessResponse,
...
def update_meal(
    ...
    try:
        ...
        return 404, NotFoundResponse(
            error="Not found",
            detail=f"Meal with id {payload.id} not found"
        )
    except IntegrityError as e:
        if 'unique constraint' in str(e).lower():
            return 409, ErrorResponse(
        ...
        import logging
        logger = logging.getLogger(__name__)
        logger.error(f"Unexpected error in update_meal: {e}", exc_info=True)
...
def get_history_by_date_and_meal_name(
        ...
        from_date: Optional[datetime.date] = Query(None),
        to_date: Optional[datetime.date] = Query(None),
        meal_type: Optional[str] = Query(None, alias="mealType"),
):
    """
