...
async def example_with_url():
    ...
    result = await food_analysis_service.analyze_food_image(
        food_images[0], additional_context="Это завтрак, порция средняя"
    )
...
async def example_with_local_image():
    ...
    if os.path.exists(local_image_path):
        ...
        result = await food_analysis_service.analyze_food_image(
            image_bytes, additional_context="Домашняя еда"
        )
        ...
        print(
            f"Файл {local_image_path} не найден. Пропускаем пример с локальным файлом."
        )
...
async def example_custom_prompt():
    ...
    # URL изображения
    spicy_food = (
        "https://images.unsplash.com/photo-1589301760014-d929f3979dbc"  # Острая еда
    )
